...
public class InventoryDaoImpl implements InventoryDao{
	...
	public InventoryProductDto getProduct(int productId) throws SQLException, ProductNotFoundException {
		...
		String sql="select p.id,p.name,p.description,p.price,i.quantityinstock,i.laststockupdate from product p join inventory i on p.id=i.id where p.id=?";
...
//			Product p=new Product();
	...
	public void insertInventory(int id, LocalDate now,int quantity) throws SQLException {
		Connection conn=DBUtil.getDBConn();
		String sql="insert into inventory(quantityinstock,laststockupdate,product_id) values(?,?,?)";
		PreparedStatement pstmt=conn.prepareStatement(sql);
	...
	public List<InventoryProductDto> getAllProducts() throws SQLException {
		...
		String sql="select p.id,p.name,p.price,i.quantityinstock,i.laststockupdate from product p join inventory i on p.id=i.product_id";
		PreparedStatement pstmt=conn.prepareStatement(sql);
		ResultSet rst=pstmt.executeQuery();
		while(rst.next()) {
			int productID=rst.getInt("id");
	...
	public void updateStock(int id, LocalDate now, int quantity) throws SQLException {
		Connection conn=DBUtil.getDBConn();
		String sql="update inventory set quantityinstock=?,laststockupdate=? where product_id=?";
		PreparedStatement pstmt=conn.prepareStatement(sql);
		pstmt.setInt(1,quantity);
		pstmt.setString(2,now.toString());
		pstmt.setInt(3,id);
	...
	public List<InventoryProductDto> getLowStockProducts(int threshold) throws SQLException {
		Connection conn=DBUtil.getDBConn();
		List<InventoryProductDto> list=new ArrayList<InventoryProductDto>();
		String sql="select p.id,p.name,p.price,i.quantityinstock,i.laststockupdate from product p join inventory i on p.id=i.product_id where i.quantityinstock<?";
		PreparedStatement pstmt=conn.prepareStatement(sql);
	...
	public List<InventoryProductDto> getOutOfStockProducts() throws SQLException {
		Connection conn=DBUtil.getDBConn();
		List<InventoryProductDto> list=new ArrayList<InventoryProductDto>();
		String sql="select p.id,p.name,p.price,i.quantityinstock,i.laststockupdate from product p join inventory i on p.id=i.product_id where i.quantityinstock=0";
		PreparedStatement pstmt=conn.prepareStatement(sql);
		ResultSet rst=pstmt.executeQuery();
		while(rst.next()) {
			int productID=rst.getInt("id");
